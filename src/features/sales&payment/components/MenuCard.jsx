...
const MenuCard = forwardRef(({ img="", category, price,name }, ref) => {
  return (
    <div className="relative w-52 rounded-xl  flex  flex-col items-center justify-around gap-y-2  h-fit p-1.5 bg-white shadow-2xs outline-1 outline-gray-300 ">
      ...
      <button className="absolute right-3 bottom-14 cursor-pointer p-2 rounded-full bg-slate-900 w-fit text-white " ref={ref}>  
   ...
   <div className="flex justify-around items-center mt-2 gap-x-3.5">
    <div className="flex flex-col">
        ...
        <h4 className="font-light bg-slate-900/45 w-fit px-2 text-sm text-white rounded-sm">{category}</h4>
        </div>
    <h4>{price} </h4>
   </div>
